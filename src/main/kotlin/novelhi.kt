...
fun main() {
    ...
    val urlFormat = "https://novelhi.com/s/I-Am-the-Fated-Villain/%d"
    val name = "The Fated Villain 194"
    val min = 194
    val max = 639
    ...
    File("/home/navn/temp/"+name.replace(' ','_')+".txt").bufferedWriter().use {
        for (i in min..max) {
            ...
            val content = WebDriverWait(chromeDriver, Duration.ofMinutes(3))
                .until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("#readcontent > div"))).text
            it.write("\n\nChapter $i:\n\n")
            it.write(content);
        }
    }
